...
import { createSession, getSession, updateSession, calculateCompleteness, listSessions } from "./storage.js";
import { REQUIREMENT_ANALYZER_PROMPT, QUESTION_GENERATOR_PROMPT, GAP_ANALYZER_PROMPT, SPEC_COMPILER_PROMPT, buildAnalyzerInput, buildQuestionGeneratorInput, buildGapAnalyzerInput, buildSpecCompilerInput } from "./prompts.js";
...
server.tool("spec_start_session", "Start a new spec clarification session from a rough requirement. Returns session ID and initial clarifying questions.", {
    ...
    try {
        ...
        const clarifications = analysis.questions.map((q, i) => ({
...
server.tool("spec_answer_questions", "Provide answers to clarifying questions. Returns updated completeness and any follow-up questions.", {
...
server.tool("spec_get_gaps", "Analyze gaps in the current session and get recommendations for improving completeness.", {
...
server.tool("spec_generate", "Generate the final structured specification from a clarification session.", {
...
server.tool("spec_get_status", "Get the current status of a clarification session.", {
...
server.tool("spec_list_sessions", "List all active clarification sessions.", {}, async () => {
...
function formatSpecAsMarkdown(spec, session) {
    const lines = [
        `# ${spec.title}`,
        "",
        ...
        "## User Flow",
        "",
        ...spec.userFlow.map(step => `${step.step}. **${step.actor}** → ${step.action} → *${step.outcome}*`),
        "",
        "---",
        "",
        "## Features",
